CREATE OR REPLACE PROCEDURE HRBZLS."SP_PRINT_CFTZD_OCX" (
...
select rlid,
       fgetopername(MICPER) || '[' || MICPER || ']' 催费员,
       rlmid,
       to_char(sysdate, 'YY') 年,
       ...
       rlmcode 户号,
       rlcname 户名,
       to_char(rlprdate,'YYYY-MM-DD') 上期抄表时间,
       to_char(rlrdate,'YYYY-MM-DD') 本期抄表时间,
       rlscode 上期表码,
       rlecode 本期抄码,
       rlsl 本期水量,
       rlje 本期水费,
       ...
       '正常' 水表状态,
       tools.fformatnum((select tools.fformatnum(nvl(sum(pddj), 0), 2)
                          from pricedetail
                         where pdpscid = 0
                           and pdmethod = 'dj1'
                           and pdpfid = RLPFID),
