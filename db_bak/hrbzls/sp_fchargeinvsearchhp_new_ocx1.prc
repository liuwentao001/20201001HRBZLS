CREATE OR REPLACE PROCEDURE HRBZLS."SP_FCHARGEINVSEARCHHP_NEW_OCX1" (
p_batch in varchar2,
...
select * from (
           ...
           '    '||tools.fuppernumber(sum(pdje)+sum(pdznj) )              C10             ,--合计金额大写            C10
         ...
         '总计:'|| TO_CHAR( SUM(pdje)+sum(pdznj ),'9999999.00')                                                         C32             ,--手续费                  C32
            ''                                                        C33             ,--上期抄表日期    （本次交钱）        C33
      '上次余数:'||   replace( tools.fformatnum(max(PLSAVINGQC), 2)   ,'-.','-0.')                                                    C34             ,--抄表日期  (上次结余)             C34
        '本次余数:'||   replace( tools.fformatnum(max(PLSAVINGQM), 2)   ,'-.','-0.')                                                    C35             ,--账务日期 (本次结余)               C35
            '暂无启用'                                                         C36             ,--抄表月份                C36
          ...
          case
             when sum(RLADDSL) > 0 then
              '调表：' || '【余量为' || tools.fformatnum(max(RLADDSL), 0) || '】(补)'
...
,'|/','|') into v_tempstr   from dual;
    ...
    INSERT INTO PRINTLISTTEMP_new VALUES (I,v_tempstr);
    end loop;
    close c_dt;
    v_hd :=  trim(to_char(lengthb( v_constructhd||v_constructdt ),'0000000000'))||
        ...
        v_constructdt||
        v_contentstrorder  ;
     INSERT INTO PRINTLISTTEMP_new VALUES (1,v_hd);
