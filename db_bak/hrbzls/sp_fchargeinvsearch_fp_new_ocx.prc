CREATE OR REPLACE PROCEDURE HRBZLS."SP_FCHARGEINVSEARCH_FP_NEW_OCX" (
...
--分票(销帐)
          ...
          replace(   tools.fuppernumber(DECODE(PLCD,'DE',1,-1)*sum(pdje)+DECODE(PLCD,'DE',1,-1)*sum(pdznj)+max(PLSAVINGBQ ) )   ,'-.','-0.')            C10             ,--合计金额大写            C10
            replace( tools.fformatnum(DECODE(PLCD,'DE',1,-1)*sum(pdje)+DECODE(PLCD,'DE',1,-1)*sum(pdznj)+max(PLSAVINGBQ ) - SUM(CASE WHEN MIIFTAX='Y' AND PDPIID='01'  THEN   PDJE  ELSE 0 END)  ,2)   ,'-.','-0.')                  C11             ,--合计金额                C11
           ...
           '抄表员：'||    max(FGETBFRPER_smfid(mibfid,mismfid))||  max(case when mistatus in ('13','21') then '  电话：'||FGETBFRPERtel_smfid(mibfid,mismfid) else '' end)                                                          C26             ,--抄表员              C26
            ...
            ''                  C28             ,--应收单价                C28
           replace(  tools.fformatnum(DECODE(PLCD,'DE',1,-1)*sum(pdje),2)   ,'-.','-0.')                                      C29             ,--水费金额                C29
            '                                '                                                         C30             ,--余量                    C30
          '违约金:'|| replace( tools.fformatnum( DECODE(PLCD,'DE',1,-1)*sum(pdznj)  ,2)  ,'-.','-0.')           C31             ,--滞纳金                  C31
           '                                '                                                        C32             ,--手续费                  C32
           '                                '                                                      C33             ,--上期抄表日期    （本次交钱）        C33
          ...
          replace(  tools.fformatnum(DECODE(PLCD,'DE',1,-1)*sum(pdje) - SUM(CASE WHEN MIIFTAX='Y' AND PDPIID='01'  THEN   DECODE(PLCD,'DE',1,-1)*PDJE  ELSE 0 END) ,2)  ,'-.','-0.')                     C37  ,--合计应收1                            C37
            '                                '                                                         C38             ,--收费方式                C38
            '                                '                                                         C39             ,--水费明细3               C39
           '                                '                                                     C40             ,--预存发生明细            C40
           tools.fuppernumber(DECODE(PLCD,'DE',1,-1)*sum(pdje) - SUM(CASE WHEN MIIFTAX='Y' AND PDPIID='01'  THEN   DECODE(PLCD,'DE',1,-1)*PDJE  ELSE 0 END) )                                C41             ,--应收金额大写           C41
