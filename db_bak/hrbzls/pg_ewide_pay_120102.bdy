CREATE OR REPLACE PACKAGE BODY HRBZLS."PG_EWIDE_PAY_120102" IS
  CURDATE DATE;
  ...
  BEGIN
    BEGIN
      SELECT * INTO V_RL FROM RECLIST RL WHERE RL.RLID = P_RLID;
      IF V_RL.RLYSCHARGETYPE IN ('T', 'M') THEN
        RETURN 0;
      END IF;
      IF F_GETIFZNJ(V_RL.RLMID) = 'N' THEN
        RETURN 0;
      END IF;
    END;
    --异常反回0
  ...
  *******************************************************************************************/
  ...
  BEGIN
    ...
    VP.PMONTH    := TOOLS.FGETPAYMONTH(P_POSITION);
