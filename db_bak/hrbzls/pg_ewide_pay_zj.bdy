CREATE OR REPLACE PACKAGE BODY HRBZLS."PG_EWIDE_PAY_ZJ" IS
  CURDATE DATE;
  ...
  *******************************************************************************************/
  ...
  BEGIN
    ...
    ------以上次序不可随意变动------------------------------------------------
    VP.PIFSAVING := (CASE WHEN VP.PSAVINGBQ <> 0 THEN 'Y' ELSE 'N' END);
    VP.PPAYWAY   := P_FKFS;
  ...
  *******************************************************************************************/
  ...
  BEGIN
    ...
    V_PRC_MSG := '将应收冲正负记录插入到应收总账中';
     ...
     ------
    INSERT INTO pbparmtemp_test T (SELECT s.RLID                      ,
s.RLSMFID                   ,
s.RLMONTH                   ,
s.RLDATE                    ,
s.RLCID                     ,
...
s.RLMISAVING
  FROM reclist_1meter_tmp S);
    ...
    UPDATE RECDETAIL_TMP T
       SET (T.RDID, T.RDPAIDFLAG, T.RDPAIDDATE, T.RDPAIDMONTH, T.RDPAIDPER, T.RDMONTH) = (SELECT S.RLID,
                                                                                                 'N',
                                                                                                 NULL,
                                                                                                 NULL,
                                                                                                 NULL,
