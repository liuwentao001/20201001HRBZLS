CREATE OR REPLACE TRIGGER HRBZLS."TUB_OPERACCNTROLEFUNC" BEFORE UPDATE
OF ORFOAID,
 ORFFID
...
BEGIN
 ...
 -- PARENT "ERPFUNCTION" MUST EXIST WHEN UPDATING A CHILD IN "OPERACCNTROLEFUNC"
 ...
 if not found then
 errno := -20003;
 errmsg := 'PARENT DOES NOT EXIST IN "ERPFUNCTION". CANNOT UPDATE CHILD IN "OPERACCNTROLEFUNC".';
 raise integrity_error;
 end if;
 end if;

 -- Parent "OPERACCNT" must exist when updating a child in "OPERACCNTROLEFUNC"
 if (:new.ORFOAID is not null) and (seq = 0) then
 ...
 IF NOT FOUND THEN
 ERRNO := -20003;
 ERRMSG := 'PARENT DOES NOT EXIST IN "OPERACCNT". CANNOT UPDATE CHILD IN "OPERACCNTROLEFUNC".';
 RAISE INTEGRITY_ERROR;
 END IF;
 END IF;
