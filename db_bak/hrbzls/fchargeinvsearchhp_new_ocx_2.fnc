CREATE OR REPLACE FUNCTION HRBZLS."FCHARGEINVSEARCHHP_NEW_OCX_2" (p_batch in varchar2,p_modelno in varchar2) return varchar2  is
 --v_invprintstr clob;
...
(
           ...
           '币  '||tools.fuppernumber(sum(pdje)+sum(pdznj)+max(PLSAVINGBQ ))                C10             ,--合计金额大写            C10
            '￥'||tools.fformatnum(max(rlje),2)                  C11             ,--合计金额                C11
           --  '币  '|| tools.fuppernumber(sum(pdje)+sum(pdznj)+max(PLSAVINGBQ ))                  C10             ,--合计金额大写            C10
           -- '￥'||tools.fformatnum(sum(pdje)+sum(pdznj)+max(PLSAVINGBQ ) ,2)                   C11             ,--合计金额                C11
            ...
            /*fGetUnitPrice_mx(max(rlid),'01,02,03,04|',4)*/ fGetUnitPrice_gtmx1(max(rlid),null)                                                  C28             ,--应收单价                C28
            fGetUnitMoney_gtmx(plid)                                      C29             ,--水费金额                C29
            '暂无启用'                                                         C30             ,--余量                    C30
            ''                                                       C31             ,--滞纳金                  C31
            ''                                                         C32             ,--手续费                  C32
            ''                                                        C33             ,--上期抄表日期    （本次交钱）        C33
            '上期转入:'||case when max(PLSAVINGQC)<0 then tools.fformatnum(max(PLSAVINGQC)*-1,2) else tools.fformatnum(max(PLSAVINGQC),2) end                                                      C34             ,--抄表日期  (上次结余)             C34
            '转入下期:'||case when max(PLSAVINGQM)<0 then tools.fformatnum(max(PLSAVINGQM)*-1,2) else tools.fformatnum(max(PLSAVINGQM),2) end                                                         C35             ,--账务日期 (本次结余)               C35
            '暂无启用'                                                         C36             ,--抄表月份                C36
          ...
          case
            ...
            '滞纳金:'||tools.fformatnum(max(PLZNJ),2)                                                       C44             ,--实缴滞纳金4           C44
            '本次应缴:'||tools.fformatnum(max(plje)+max(PLZNJ)-max(PLSAVINGQC),2)                                                      C45             ,--应缴水费5           C45
            /*'本次实缴:'||*/tools.fformatnum(sum(pdje)+sum(pdznj)+max(PLSAVINGBQ ) ,2)                                                        C46             ,--实缴水费6           C46
            case when max(plcd)='DE' then '' else '冲红票' end                                                         C47             ,--用户预留字段7           C47
            fGetUnitMoney_xc(plid,'01')                                                  C48             ,--基本水价           C48
            fGetUnitMoney_xc(plid,'02')                                                         C49             ,--城市附加费           C49
            fGetUnitMoney_xc(plid,'03')                                                         C50             ,--水资源费          C50
            fGetUnitMoney_xc(plid,'04')                                                            C51             ,--污水处理费            C51
            ''                            C52             ,--费用项目                C52
            MAX('c2')                                                                 C53             ,--打印员编号              C53
            case when rlyschargetype='T' then maaccountno else '' end                                                       C54             ,--开户账号              C54
            case when rlyschargetype='T' then (select  smfname  from sysmanaframe where smfid=mabankid) else '' end                                                                 C55             ,--开户名                    C55
            max(PCD )                                                               C56             ,--系统预留字段1           C56
            MAX('c3')                                                        C57             ,--系统预留字段2           C57
            ''                                                        C58             ,--系统预留字段3           C58
            ''                                                         C59             ,--系统预留字段4           C59
            ''                                                         C60              --系统预留字段5           C60
     from payment,paidlist,paiddetail,reclist/* ,  pbparmtemp*/,meteraccount
   where pid=plpid and
         plid=pdid and
         plrlid=rlid and
         pmid=mamid and
...
select
            ...
            fGetOperName(MAX('c2'))                                                   C8              ,--打印员                  C8
            max(ppayee)                                          C9              ,--收费员                  C9
           '   '|| tools.fuppernumber(sum( decode(pcd,'DE',1,-1)*(ppayment -pchange)))                     C10             ,--合计金额大写            C10
            ...
            '上期转入:'||case when max(psavingqc )<0 then tools.fformatnum(max(psavingqc)*-1,2) else tools.fformatnum(max(psavingqc),2) end                                                      C34             ,--抄表日期  (上次结余)             C34
            '转入下期:'||case when max(psavingqm )<0 then tools.fformatnum(max(psavingqm)*-1,2) else tools.fformatnum(max(psavingqm),2) end                                                         C35             ,--账务日期 (本次结余)
            '暂无启用'                                                         C36             ,--抄表月份                C36
            ''                 合计应收1                                                  ,--合计应收1                            C37
            '现金'                                                         C38             ,--收费方式                C38
            '暂无启用'                                                         C39             ,--水费明细3               C39
