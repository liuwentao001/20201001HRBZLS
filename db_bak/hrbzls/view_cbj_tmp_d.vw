CREATE OR REPLACE FORCE VIEW HRBZLS.VIEW_CBJ_TMP_D AS
WITH  RL_DIS AS
(
SELECT RLPRIMCODE,
       NVL(SUM(RLJE), 0) DIS_JE,
       COUNT(RLID) DIS_C,
       TO_CHAR(MIN(RLDATE), 'YYYYMMDD') DIS_M
  FROM HRBCBJ_D T,
       METERINFO MI,
       RECLIST RL
 WHERE RLPAIDFLAG = 'N'
...
/*    SELECT PMID, MAX(PDATETIME) PDATETIME, SUM(PPAYMENT) PPAYMENT
     ...
     (SELECT \*+ USE_NL(PAYMENT) ORDERED *\PMID, max(PBATCH) FROM PAYMENT
     WHERE     PREVERSEFLAG = 'N'
       AND PTRANS <> 'U'
     and PMID IN (SELECT C1 FROM HRBCBJ_D T) group by pmid )
     GROUP BY PMID*/

    SELECT PMID, MAX(PDATETIME) PDATETIME, SUM(PPAYMENT) PPAYMENT
      FROM PAYMENT X
     WHERE (PMID, PBATCH)  in
     (select PMID,  pbatch from view_last_pbatch
where PMID IN (SELECT C1 FROM HRBCBJ_D T) )
     GROUP BY PMID
)
SELECT  /*+ USE_NL(MI) ORDERED */ MIID || CHR(9) || --  水表编号C1,
  ...
  FROM BOOKFRAME BF,
       ( select METERINFO.* from METERINFO, HRBCBJ_D  where miid =  c1 ) MI,
       ( select METERDOC.* from METERDOC, HRBCBJ_D  where MDMID =  c1 )  MD,
        ( select CUSTINFO.* from CUSTINFO, HRBCBJ_D  where CIID =  c1 )   CI,
        ( select METERREAD.* from METERREAD, HRBCBJ_D  where MRMID =  c1 )   MR,
       RL_DIS    RL_DIS,
       P
 WHERE
     P.PMID(+) = MI.MIID
   AND RL_DIS.RLPRIMCODE(+) = MI.MIPRIID
   AND MI.MIID = MD.MDMID
   AND MI.MIID = CI.CIID
   AND BF.BFID = MI.MIBFID
   AND MI.MIID = MR.MRMID(+)
  AND MIID IN (SELECT C1 FROM HRBCBJ_D T)
