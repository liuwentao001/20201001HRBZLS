CREATE OR REPLACE PACKAGE BODY HRBZLS."PG_EWIDE_PRINT_01" IS
  --票据打印包
  --柜台缴费
  procedure sp_gtjf_ocx1(p_pbatch    in varchar2, --实收批次
    ...
    cursor c_dt is
      select max(pmcode) C1, --资料号                  C1
             ...
             to_char(max(t3.rlscode)) C4, --抄表起码                C4
             to_char(max(t3.rlecode)) C5, --抄表止码                C5
             '计量吨位：' ||to_char(max(t3.rlsl)) C6, --应收水量                C6
             to_char(sysdate, 'yyyy-mm-dd') C7, --打印日期                C7
             fgetopername('1146') C8, --打印员                  C8
             fgetopername(max(pper)) C9, --收费员                  C9
             '币' || '    ' ||tools.fuppernumber(max(decode(rlgroup,'1',ppayment-pchange,0))+max(decode(rlgroup,'2',ppayment-pchange,0))) C10, --合计金额大写        C10
             '￥' ||to_number(max(decode(rlgroup,'1',ppayment-pchange,0))+max(decode(rlgroup,'2',ppayment-pchange,0))) C11, --合计金额小写                C11
             fgetopername(max(pper)) C12, --开票员               C12
             max(case
             ...
             '' C21, --水表编号                C21
             '' C22, --用户编号                C22
             max(to_char(rldate, 'YYYY-MM-DD')) C23, --账务日期                  C23
             '' C24, --水表安装地址            C24
             '' C25, --表册号                  C25
             max(fgetopername(RLRPER)) C26, --抄表员              C26
             '' C27, --制据日期                C27
             '' C28, --应收单价                C28
             '' C29, --水费金额                C29
             '' C30, --余量                    C30
             '滞纳金：' ||max(plznj) C31, --滞纳金                  C31
             '' C32, --手续费                  C32
             '' C33, --上期抄表日期    （本次交钱）        C33
             max(case when psavingbq <>0 then  psavingqc else 0 end ) C34, -- 上次结余           C34
             max(case when psavingbq <>0 then  psavingqm else 0 end ) C35, --本次结余             C35
             '用水类别：' ||(case when max(MIIFMP)='N' then max(FGETPRICEFRAME_jh(rlpfid)) else '混合用水' end) C36, --用水类别                C36
             '应收小计：' ||max(rlje) C37, --应收小计                          C37
             '' C38, --收费方式                C38
             '' C39, --水费明细3               C39
             '' C40, --预存发生明细            C40
             '' C41, --应收金额大写           C41
             ...
             '净水价:' ||(case when max(MIIFMP)='N' then tools.fformatnum(sum(case
                              ...
                              2) else '复合水价' end) C47, --基本水费单价 C47
             '' C48, --用户预留字段8           C48
             '' C49, --用户预留字段9           C49
             '' C50, --用户预留字段10          C50
             '' C51, --用户预留字段10         C51
             '净水费:' ||tools.fformatnum(sum(case
                                    when pdpiid = '01' then
                                     pdje
                                  end),
                              2) C52, --基本水费 C52
             '代征附加费：' ||tools.fformatnum(sum(case
                                    when pdpiid = '03' then
                                     pdje
                                  end),
                              2) C53, --附加费  C53
             '代征污水处理费：' ||tools.fformatnum(sum(case
                                    when pdpiid = '02' then
                                     pdje
                                  end),
                              2) C54, --污水费  C54
             '' C55, --表序号                    C55
             tools.fformatnum(sum(case
                                    when pdpiid = '04' then
                                     pdje
                                  end),
                              2) C56, --垃圾费
             '环卫费：' ||min(to_char(rlprdate,'YYYY.MM')) || '至' || max(to_char(rlrdate,'YYYY.MM')) C57, --垃圾费月份           C57
             '' C58, --系统预留字段3           C58
             '' C59, --系统预留字段4           C59
             '.' C60 --系统预留字段5           C60

        from payment t, paidlist t1, paiddetail t2, reclist t3, meterdoc t4,meterinfo t5
       where pid = plpid
         and plid = pdid
         and rlid = plrlid
         and rlmid = mdmid
         and miid = rlmid
         and pbatch = p_pbatch
       group by pbatch, rlmcode, rlmonth, RLMRID
       order by pbatch, rlmcode, rlmonth, RLMRID;
  ...
  procedure sp_gtjf_ocx(p_pbatch    in varchar2, --实收批次
    ...
    from (
      select max(pmcode) C1, --资料号                  C1
             ...
             '' C58, --系统预留字段3           C58
             max(rlmonth) C59, --系统预留字段4           C59
             max(pbatch) C60 --系统预留字段5           C60
...
(select rlnum,
             ...
             (case when max(MIIFMP)='N' then max(FGETPRICEFRAME_jh(rlpfid)) else '混合用水' end) 用水类别, --用水类别                C36
             sum(case
                                    when pdpiid = '01' or pdpiid = '02' or pdpiid = '03' then
                                     pdje
                                  end) 应收小计, --应收小计                          C37

             (case when max(MIIFMP)='N' then tools.fformatnum(sum(case
                                    when pdpiid = '01' then
                                     pddj
                                  end),
       ...
       null,null,null,null,null,null,null,null,null,null,
       null,null,null,null,null,null,null,null,null,null,
       null,null,null,null,null,null,null,null,null,'9999999999'
       from dual
       )
       order by  c60, c1, c59
