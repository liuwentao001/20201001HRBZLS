CREATE OR REPLACE PROCEDURE HRBZLS."SP_CUSTINFOPRINT_OCX" (
...
(SELECT  MI.MIID AS CICODE,
  ...
  MAX(MI.MICODE) AS  MICODE ,
         ...
         max(case when bf.bfrcyc=1 then 'Ã¿' when bf.bfrcyc=2 and mod(to_number(substr(bf.BFNRMONTH,6,2)),2)=1 then 'µ¥' when bf.bfrcyc=2 and mod(to_number(substr(bf.BFNRMONTH,6,2)),2)=0 then 'Ë«' else 'µ¥' end)/*MAX(CI.CICONNECTTEL)*/ AS  MIPOSITION ,
         ...
         '16' AS  Ô¤Áô×Ö¶Î17 ,
         '28' AS  Ô¤Áô×Ö¶Î18 ,
          max(case when bf.bfrcyc=1 then 'Ã¿' when bf.bfrcyc=2 and mod(to_number(substr(bf.BFNRMONTH,6,2)),2)=1 then 'µ¥' when bf.bfrcyc=2 and mod(to_number(substr(bf.BFNRMONTH,6,2)),2)=0 then 'Ë«' else 'µ¥' end) AS  Ô¤Áô×Ö¶Î19 ,
         /*to_char(max(MIINSDATE),'yyyy.mm.dd')*/fGetxym(max(MI.MICODE),8) AS  Ô¤Áô×Ö¶Î20
