CREATE OR REPLACE PACKAGE BODY HRBZLS."ZHONGBE_bak" is
    ...
    insert into bankchklog_new
      (SELECT trim(to_char(bankchklog.nextval, '0000000000')) id,
         ...
         FROM (select A.CHKDATE,
                      ...
                      NULL,
                      NULL
                 FROM (SELECT DISTINCT (trunc(T1.PCHKDATE)) CHKDATE
                         FROM PAYMENT T1
...
BEGIN
  --【哈尔滨】每天凌晨1点检查昨日银行是否扎帐，如果未扎帐则自动扎帐
 -- V_CHKDATE := TRUNC(SYSDATE) - 1;
  select trunc(to_date('20140727','yyyymmdd'))  into V_CHKDATE from dual;
