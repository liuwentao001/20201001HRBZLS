CREATE OR REPLACE TRIGGER HRBZLS."TUB_METERCHKPLANPARA" BEFORE UPDATE
OF MCPPSORT,
 MCPPCALIBER
...
BEGIN
 ...
 -- PARENT "METERCALIBER" MUST EXIST WHEN UPDATING A CHILD IN "METERCHKPLANPARA"
 IF (:NEW.MCPPCALIBER IS NOT NULL) AND (SEQ = 0) THEN
 ...
 if not found then
 errno := -20003;
 errmsg := 'PARENT DOES NOT EXIST IN "METERCALIBER". CANNOT UPDATE CHILD IN "METERCHKPLANPARA".';
 raise integrity_error;
 end if;
 end if;

 -- Parent "METERSORTFRAME" must exist when updating a child in "METERCHKPLANPARA"
 if (:new.MCPPSORT is not null) and (seq = 0) then
 ...
 IF NOT FOUND THEN
 ERRNO := -20003;
 ERRMSG := 'PARENT DOES NOT EXIST IN "METERSORTFRAME". CANNOT UPDATE CHILD IN "METERCHKPLANPARA".';
 RAISE INTEGRITY_ERROR;
 END IF;
 END IF;
