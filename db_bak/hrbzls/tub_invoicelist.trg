CREATE OR REPLACE TRIGGER HRBZLS."TUB_INVOICELIST" BEFORE UPDATE
OF ILID,
 ILTYPE,
 ILCD
ON INVOICELIST FOR EACH ROW
DECLARE
 INTEGRITY_ERROR EXCEPTION;
 ERRNO INTEGER;
 ERRMSG CHAR(200);
 DUMMY INTEGER;
 FOUND BOOLEAN;
 SEQ NUMBER;
 -- DECLARATION OF UPDATECHILDPARENTEXIST CONSTRAINT FOR THE PARENT "INVOICETYPE"
 CURSOR CPK1_INVOICELIST(VAR_ILTYPE VARCHAR) IS
 SELECT 1
 FROM INVOICETYPE
 WHERE ITID = VAR_ILTYPE
 AND VAR_ILTYPE IS NOT NULL;
 -- DECLARATION OF UPDATECHILDPARENTEXIST CONSTRAINT FOR THE PARENT "ACCOUNTCD"
 CURSOR CPK2_INVOICELIST(VAR_ILCD VARCHAR) IS
 SELECT 1
 FROM ACCOUNTCD
 WHERE ACDID = VAR_ILCD
 AND VAR_ILCD IS NOT NULL;

BEGIN
  if nvl(fsyspara('data'),'N')='Y' then
     return;
  end if;
 SEQ := INTEGRITYPACKAGE.GETNESTLEVEL;
 -- PARENT "INVOICETYPE" MUST EXIST WHEN UPDATING A CHILD IN "INVOICELIST"
 IF (:NEW.ILTYPE IS NOT NULL) AND (SEQ = 0) THEN
 OPEN CPK1_INVOICELIST(:NEW.ILTYPE);
 FETCH CPK1_INVOICELIST INTO DUMMY;
 FOUND := CPK1_INVOICELIST%FOUND;
 close CPK1_INVOICELIST;
 if not found then
 errno := -20003;
 errmsg := 'PARENT DOES NOT EXIST IN "INVOICETYPE". CANNOT UPDATE CHILD IN "INVOICELIST".';
 raise integrity_error;
 end if;
 end if;

 -- Parent "ACCOUNTCD" must exist when updating a child in "INVOICELIST"
 if (:new.ILCD is not null) and (seq = 0) then
 open CPK2_INVOICELIST(:new.ILCD);
 fetch CPK2_INVOICELIST into dummy;
 found := CPK2_INVOICELIST%FOUND;
 CLOSE CPK2_INVOICELIST;
 IF NOT FOUND THEN
 ERRNO := -20003;
 ERRMSG := 'PARENT DOES NOT EXIST IN "ACCOUNTCD". CANNOT UPDATE CHILD IN "INVOICELIST".';
 RAISE INTEGRITY_ERROR;
 END IF;
 END IF;


-- ERRORS HANDLING
EXCEPTION
 WHEN INTEGRITY_ERROR THEN
 RAISE_APPLICATION_ERROR(ERRNO, ERRMSG);
END;
/

