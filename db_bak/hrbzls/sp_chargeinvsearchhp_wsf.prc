...
begin
  ...
  open o_base for
    select max(rlmcode) C1, --资料号                  C1
           ...
           sum(decode(pdpiid, '02', pdsl, 0)) C6, --应收水量                C6
           to_char(SYSDATE, 'yyyy-mm-dd') C7, --打印日期                C7
           fGetOperName(MAX(c2)) C8, --打印员                  C8
           fGetOperName(max(ppayee)) C9, --收费员                  C9
              tools.fuppernumber(sum(decode(pdpiid, '02', pdJE, 0)))  C10, --合计金额大写            C10
            sum(decode(pdpiid, '02', pdsl, 0)) C11, --合计金额                C11
          ...
          /* case
           ...
           MAX(rlprimcode) C48, --用户预留字段8           C48
           '苏州吴中供水有限公司' C49, --用户预留字段9           C49
           '32201997591050682841' C50, --用户预留字段10          C50
           '建行苏州吴中经济开发区' C51, --应收账流水号            C51
           '' C52, --费用项目                C52
