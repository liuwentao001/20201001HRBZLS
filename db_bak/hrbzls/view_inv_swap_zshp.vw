CREATE OR REPLACE FORCE VIEW HRBZLS.VIEW_INV_SWAP_ZSHP AS
SELECT '' ID, --出票流水号
       '' ISID, --发票流水号
       '' ISPCISNO, --票据批次||号码
       ...
       SYSDATE pdate,  --打印时间
      (CASE
         WHEN MAX(RLTRANS) in ('u','v','13','14','21') THEN
          MAX(RLCNAME)
         ELSE
          MAX(MINAME)
       END) RLCNAME, --用户名称
       ...
       max(rd.dj) dj,          --总单价
       --max(rd.dj1) dj1,         --单价1
       fun_getjtdqdj(max(mipfid),max(MIPRIID),max(miid),'1') dj1,
       --fgetwsf(max(mipfid)) dj2,         --单价2
       fgetwsfss(max(mi.miid)) dj2,         --单价2   --20160715 wlj 取污水费实收单价
       max(rd.dj3) dj3,         --单价3
       max(rd.dj4) dj4,         --单价4
/*       max(rd.dj5) dj5,         --单价5
       max(rd.dj6) dj6,         --单价6
       max(rd.dj7) dj7,         --单价7*/
       (case when MAX(RLTRANS) in ('13','14','21') and MAX(RLINVMEMO)='A' then MAX(rd.YSDJ1) else null end ) dj5,         --单价5
       (case when MAX(RLTRANS) in ('13','14','21') and MAX(RLINVMEMO)='A' then MAX(rd.YSDJ2) else null end ) dj6,         --单价6
       (case when MAX(RLTRANS) in ('13','14','21') and MAX(RLINVMEMO)='A' then MAX(rd.YSDJ3) else null end ) dj7,         --单价7
       --max(rd.dj8) dj8,         --单价8
       --max(rd.dj9) dj9,         --单价9
       ...
       '' CZPER, --冲正人员
       '' CZDATE, --冲正日期
       (select max(fgetopername(bfrper)) from bookframe where bfid = (select max(mibfid) from meterinfo where miid =RLPRIMCODE) ) JZDID, --进账单流水//2017.09.18 改为核对人
       0 yjbss,     --预计表示数
       ...
       (CASE WHEN MAX(RLTRANS)  in ('13','14','21') then FGETSYSCHARLIST('补缴类别',PG_EWIDE_INVMANAGE_01.fgetinvmemo(max(rlid),'rlinvmemo')) else null end) MEMO12, --备注12  发票备注 追补类别
       (CASE WHEN MAX(RLTRANS)  in ('13','14','21') then PG_EWIDE_INVMANAGE_01.fgetinvmemo(max(rlid),'rlmemo') else null end) MEMO13, --备注13  备注
       ...
       FGETOPERNAME(fgetpboper) MEMO17, --备注17  缴费机构改为与打印人员一样
       PG_EWIDE_INVMANAGE_01.fgetinvdeatil_zs(CONNSTR(RLID),'JT',6) MEMO18, --备注18
       '' MEMO19, --备注19
       FGETBFORDER(RL.RLPRIMCODE) MEMO20 --备注20
