CREATE OR REPLACE PACKAGE BODY HRBZLS."PG_EWIDE_PAY_LYG" IS
  CURDATE DATE;
    ...
    LOOP
      ...
      UPDATE RECLIST_1METER_TMP T
         SET T.RLID = FGETSEQUENCE('RECLIST'),
             ...
             T.RLDATE  = TOOLS.FGETRECDATE(T.RLMSMFID), --当前  */
...
/*    -----------------------------------------
    ...
    --add 2013.01.17 更新reclist_charge_01表中的rdpaidmonth字段
  ...
  *******************************************************************************************/
  ...
  BEGIN
    ...
    IF P_RLJE <> V_REC_TOTAL OR P_ZNJ <> V_ZNJ_TOTAL OR
      ...
      V_MSG := '传入金额:' || P_RLJE || ',' || P_ZNJ || ',' || P_SXF;
      V_MSG := V_MSG || ' 实际不符:' || V_REC_TOTAL || ',' || V_ZNJ_TOTAL || ',' ||
               V_SXF_TOTAL;
  ...
  EXCEPTION
    WHEN ERR_NOMATCH THEN
      RAISE_APPLICATION_ERROR(ERRCODE, V_MSG);
      RETURN - 1;
    WHEN ERR_JE THEN
      RETURN - 2;
    WHEN OTHERS THEN
  ...
  *******************************************************************************************/
  FUNCTION F_PAYBACK_BY_PMID(P_PAYID    IN PAYMENT.PID%TYPE,
    ...
    CURSOR C_RLTEMP IS
      SELECT * FROM RECLIST_1METER_TMP;
    V_RLTEMP RECLIST_1METER_TMP%ROWTYPE;
  ...
  BEGIN
    ...
    SET T.RLID = FGETSEQUENCE('RECLIST'),
        ...
        T.RLDATE  = TOOLS.FGETRECDATE(MI.MISMFID), --当前    *\
        T.RLMONTH        = PM.PMONTH, --当前              帐务月份
        T.RLDATE         = PM.PDATE, --当前              帐务日期
        T.RLREADSL       = 0 - T.RLREADSL, --抄见水量
        T.RLENTRUSTBATCH = NULL, --托收代扣批号
    ...
    LOOP
      ...
      SP_RECLIST_CHARGE_01(V_RLTEMP.RLID, '1');
    END LOOP;
