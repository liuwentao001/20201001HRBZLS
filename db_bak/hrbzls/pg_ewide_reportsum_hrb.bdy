...
/*        DBMS_SNAPSHOT.REFRESH(
        ...
        insert into MV_RECLIST_CHARGE_02 
        SELECT COUNT (*) C,
         COUNT (RDSL) RDSL_C,
         COUNT (RDJE) RDJE_C,
         COUNT (RDDJ) RDDJ_C,
         ...
         RDMID METERNO,
         SUM (RDSL) WATERUSE_S,
         -- SUM(DECODE(RDPIID, '01', RDSL, 0)) WATERUSE, --  总水量  20141201调整,因补缴的水量存在问题
         SUM (CASE WHEN RDPIID = '01' AND RDJE <> 0 THEN RDSL ELSE 0 END)
            WATERUSE,                                                  --  总水量
         SUM (CASE WHEN RDPIID = '02' AND RDJE <> 0 THEN RDSL ELSE 0 END) WSSL, --污水水量
         ...
         SUM (
            CASE WHEN RDPIID = '01' AND RDCLASS IN (0, 1) THEN RDDJ ELSE 0 END)
            USER_DJ1,                                                --  阶梯1单价
         ...
         SUM (
            CASE WHEN RDPIID = '01' AND RDCLASS IN (0, 1) THEN RDSL ELSE 0 END)
            USE_R1,                                                  --  阶梯1SL
         SUM (DECODE (RDPIID, '01', DECODE (RDCLASS, 2, RDSL, 0), 0)) USE_R2, --  阶梯2SL
         SUM (DECODE (RDPIID, '01', DECODE (RDCLASS, 3, RDSL, 0), 0)) USE_R3, --  阶梯3SL
         SUM (
            CASE WHEN RDPIID = '01' AND RDCLASS IN (0, 1) THEN RDJE ELSE 0 END)
            CHARGE_R1,                                               --  阶梯1金额
         ...
         max (DECODE (RDPIID, '01', 1, 0)) C_CHARGE,                  --  笔数*/
         ...
         0 T
         FROM RECDETAIL rd
         where rdid >= (select min(rlid) from reclist rl where rl.rlmonth = A_MONTH ) and
               rdid not like 's%'  and
               not exists(select 1 from mv_reclist_charge_02 mv where rd.rdid = mv.rdid) 
...
/*  ALTER TABLE RPT_SUM_READ NOLOGGING;
  ALTER TABLE RPT_SUM_DETAIL NOLOGGING;
  ALTER TABLE RPT_SUM_CHARGE NOLOGGING;
  ALTER TABLE RPT_SUM_TOTAL NOLOGGING;*/
...
/* --add 20141024 hebang
 ...
 --   COMMIT;
    ...
    --LIQIZHU 20131010 RPT_SUM_DETAIL表改为RPT_SUM_READ
...
and rowid in 
        (select min(rowid) from RPT_SUM_READ
       ...
       SET (T.SP1,
           ...
           (select
             ...
             WHERE  P2 = A_MONTH and PTYPE = '21'
               AND D3 = t.cby )
     WHERE
               id in (select min(id) from RPT_SUM_READ x where U_MONTH = A_MONTH group by cby  ) --放第一条
               and U_MONTH = A_MONTH;
...
-----------------------------------------------------------------------------------------------*/
  ...
  BEGIN
    ...
    INSERT INTO RPT_SUM_DETAIL
      (TPDATE,           --生成日期   
       U_MONTH,          --账务月份
       OFAGENT,          --营业所,
       CHAREGETYPE,      --收费方式（M：走收总表，X：坐收户表）
       WATERTYPE,        --水价类别
       T16,              --应收事务（rectrans表trmemo='使用'）
       t17,              --水表类别（H,D ）
       m50)              --维度冲正标志1冲往月 0-正常 2-冲当月
      SELECT SYSDATE,
             A_MONTH, --账务月份
             OFAGENT, --营业所
             --AREA, --区域
  ...
  /*                    (case when rl.RLREVERSEFLAG = 'Y' AND RL.RLMONTH > RL.RLSCRRLMONTH and rl.rlmonth='2015.03'    then 1  --1代表冲往月
     when rl.RLREVERSEFLAG = 'Y' AND RL.RLMONTH =  RL.RLSCRRLMONTH  and rl.rlmonth='2015.03'  then 2  --2代表冲本月 
  when rl.RLREVERSEFLAG = 'Y' AND RL.RLMONTH =  RL.RLSCRRLMONTH  and rl.rlmonth <> '2015.03'  then 1  --2代表冲本月
      else 0 end ) x41, -- 20140901添加维度M50  值1代表冲往月 0 代表正常*/
             SUM(WATERUSE) C1,    --应收总水量
             SUM(USE_R1) C2,      --阶梯1
             SUM(USE_R2) C3,      --阶梯2
...
*/
    ...
    exception             
      when others then
        RAISE_APPLICATION_ERROR(-20001, '运行错误!' || sqlerrm);               
    end;
    ---补充VIEW_METER_PROP不存在的维度----
         ...
         SET (--C1,  --应收总水量
             ...
             (SELECT --X1,
               ...
               WHERE U_MONTH = T1
                 ...
                 AND T.OFAGENT = T5
                 AND T.T16 = T6    --应收事务
                 and T.t17 = t8    --水表类别（H,D）
                 and t.m50=x41 )   --维度冲正标志1冲往月 0-正常 2-冲当月
       WHERE T.U_MONTH = A_MONTH and 
             exists( select 1
                       from RPT_SUM_TEMP
                      where t.u_month = t1 and
                            t.watertype = t3 and
                            t.charegetype = t4 and
                            t.ofagent = t5 and
                            t.t16 = t6 and 
                            t.t17 = t8 and
                            t.m50 = x41
    ...
    --4.增加消防水鹤的  by  20151203 ralph
    execute immediate 'truncate table RPT_SUM_TEMP';		
    begin
      INSERT INTO RPT_SUM_TEMP
          (T1,
           --T2,    byj comment 2016.7.1
           T3,
           T4,
           T5,
           T6,
          ...
          SELECT A_MONTH U_MONTH,
                 --substr(rlbfid, 1, 5), -- M.AREA, byj comment 2016.7.1
                 rlpfid, --m.WATERTYPE WATERTYPE,
                 --rlrper, -- M.CBY,
                 rlyschargetype, --M.CHARGETYPE,
                 rlsmfid, -- M.OFAGENT,
                 ...
                 SUM(0) X39,            --总销账_费用项目3
                 SUM(0) X40,            --总销账_费用项目4
                 SUM(0) X41,          --总销账_阶梯1 金额
                 SUM(0) X42,          --总销账_阶梯2 金额
                 SUM(0) X43,          --总销账_阶梯3 金额
                 SUM(1) X44,                  --总销账_笔数
                 SUM(0) X45,            --总销账_代收费4
                 SUM(0) X46,            --总销账_代收费5
                 SUM(0) X47,            --总销账_代收费6
                 ...
                 SUM(0) X62,
                 0 X63,
                 SUM(case when MISTATUS in ('29','30' )  then 0 else 1 end )   m11, --按表件数
                 SUM(case when MISTATUS in ('29','30' )  then 0 else 0 end )   m12, --按表金额
                 SUM(case when MISTATUS in ('29','30' )  then 1 else 0 end )    m13, --按人件数
                 SUM(case when MISTATUS in ('29','30' )  then 0 else 0 end )  m14, --按人金额
                 SUM(case when MISTATUS in ('29','30' ) then 0 else rlsl end )   m15, --按表水量
                 SUM(case when MISTATUS in ('29','30' )  then rlsl else 0 end )  m16, --按人水量
           ...
           WHERE RLMID=MIID          
             ...
             AND RL.RLMONTH = A_MONTH  --实收账月份
               and rltrans not in ( 'u', 'v', '13', '14', '21','23')
           --GROUP BY substr(rlbfid, 1, 5)  , rlyschargetype , rlpfid , rlrper , rlsmfid , rllb;
             GROUP BY rlyschargetype , rlpfid , rlsmfid , rllb,rltrans ;
           commit;
...
commit;
   ...
   begin
		 update RPT_SUM_detail set sp21 = 1
			 WHERE
								 id in (select min(id) from RPT_SUM_detail x where   U_MONTH = a_month
...
/*
 ...
 --   20150402
    ...
    INSERT INTO RPT_SUM_TEMP
     ...
     from (
      ...
      GROUP BY PPAYWAY, PPOSITION, nvl(PPAYEE, '00000'), M.CHARGETYPE, M.OFAGENT,M.WATERTYPE,  m.MILB,
           (case when P.PREVERSEFLAG = 'Y' AND P.PMONTH > P.PSCRMONTH    then 1  --1代表冲往月
                    when P.PREVERSEFLAG = 'Y'  AND P.PMONTH =  P.PSCRMONTH    then 2  --2代表冲本月  
                    else 0 end )  -- 20140901添加维度M50  值1代表冲往月 0 代表正常
      union 
       select 
             PPAYWAY,
             PPOSITION,
  ...
  BEGIN
    ...
    INSERT INTO RPT_SUM_TOTAL
      (TPDATE, U_MONTH, OFAGENT, WATERTYPE, CHAREGETYPE,M50)
      SELECT DISTINCT --LIQIZHU 20131010 增加DISTINCT，防止从RPT_SUM_READ表中获取的维度有重复
                      SYSDATE,
                      A_MONTH, --账务月份
                      OFAGENT, --营业所
                      --AREA, --区域
    ...
    ---补充VIEW_METER_PROP不存在的维度--RPT_SUM_DETAIL--
    INSERT INTO RPT_SUM_TOTAL
      (TPDATE, U_MONTH, OFAGENT, WATERTYPE, CHAREGETYPE,M50)
      SELECT DISTINCT --LIQIZHU 20131010 增加DISTINCT，防止从RPT_SUM_DETAIL表中获取的维度有重复
                      SYSDATE,
                      A_MONTH, --账务月份
                      OFAGENT, --营业所
                      --AREA, --区域
    ...
    INSERT INTO RPT_SUM_TEMP
      ...
      SELECT A_MONTH U_MONTH,
             ...
             M.WATERTYPE,
             M.CHARGETYPE,
            (case when py.PREVERSEFLAG = 'Y' AND py.PMONTH > py.PSCRMONTH and py.pmonth=A_MONTH  then 1  --1代表冲往月
                    when py.PREVERSEFLAG = 'Y'  AND py.PMONTH =  py.PSCRMONTH  and py.pmonth=A_MONTH  then 2  --2代表冲本月 
                    when py.PREVERSEFLAG = 'Y'  AND py.PMONTH =  py.PSCRMONTH  and py.pmonth <> A_MONTH  then 1  --2代表冲本月 
                    else 0 end )  x41, -- 20140901添加维度M50  值1代表冲往月 0 代表正常
             SUM(CASE
                   WHEN PY.PTRANS = 'S' OR PY.PSCRTRANS = 'S' THEN
                    PY.PPAYMENT
...
/*通过实收的票面金额、销账金分析营业所、银行的销账情况
  蔡俊平    20140626
  byj edited on 2016.5  去掉 稽查收入 和 客服中心污水费 不参与到正常账务系统中
*/
-------------------------------------------------------------------------------------------------
PROCEDURE 财务资金统计(A_MONTH VARCHAR2) IS
  BEGIN
    ...
    select 账务月份,  
           ...
           sum(nvl(补缴水量,0)) 补缴水量,
           sum(nvl(补缴水费,0)) 补缴水费,
           sum(nvl(补缴污水费,0)) 补缴污水费,
           sum(nvl(水量,0)) 水量,                   --(不含基建、补缴)
           sum(nvl(水费,0)) 水费,                   --(不含基建、补缴)
           sum(nvl(污水量,0)) 污水量,               --(不含基建、补缴)  
           sum(nvl(污水费,0)) 污水费,               --(不含基建、补缴)
           sum(nvl(票面金额,0)) 票面金额,
           (sum(nvl(票面金额,0))-sum(nvl(总金额,0))) 预存发生,
           sum(附加费) 附加费 
      ...
      from
            ...
            AND RL.RLMID = M.METERNO
            AND RL.RLPID = P.PID               
            AND rl.rltrans not in ('21','v','23') --去掉 营销部收入、客服污水立账、稽查收入 
            ...
            AND P.PMONTH = A_MONTH             --A_month
            and substr(pposition, 1, 2) in ('02', '03')
            --去掉 营销部收入、客服中心收入、稽查处收入
            and not exists(select 1 from reclist where p.pid = reclist.rlpid and rltrans in ('21','v','23') )
            --AND P.PTRANS<>'O'  --营销部收入单独统计
            --and pposition = '0208'
            GROUP BY PPAYWAY,
                  PPOSITION,
...
-----------------------------------------------------------------------------------------
  END;
-----------------------------------------------------------------------------------------------
--每月15日自动生成gis数据  20160525
----------------------------------------------------------------------------------------------- 
PROCEDURE gis数据生成 IS
BEGIN
  DELETE VIEW_BCUSTINFO1;
  COMMIT;
  insert into VIEW_BCUSTINFO1
    select ci.ciid,
           ci.ciname,
           ci.ciadr,
           ci.cismfid,
           FGETPRICENAME(rl2.RLPFID),
           FGETSYSMANAFRAME(mi.mismfid),
           rl2.rlreadsl / rl2.yfc,
           rl2.rlmonth,
           rl2.rlreadsl
      from custinfo ci,
           meterinfo mi,
           (select rlsmfid,
                   ...
                   rlreverseflag,
                   RLPFID,
                   months_between(to_date(max(rlmonth) || '.01',
                                          'yyyy.mm.dd'),
                                  (to_date(min(rlmonth) || '.01',
                                           'yyyy.mm.dd'))) yfc,
                   max(rlmonth) rlmonth,
                   sum(decode(yf, 1, rlreadsl, 0)) rlreadsl
              from (select rl1.rlsmfid rlsmfid,
                           rl1.rlmonth rlmonth,
                           rl1.rlmid rlmid,
                           rl1.rlreadsl rlreadsl,
                           dense_rank() over(partition by rl1.rlmid order by rl1.rlmonth desc) yf,
                           rl1.rlreverseflag rlreverseflag,
                           rl1.RLPFID RLPFID
                      from (select rl.rlsmfid,
                                   rl.rlmonth,
                                   rl.rlmid,
                                   rl.RLPFID,
                                   rl.rlreverseflag,
                                   sum(rl.rlreadsl) rlreadsl
                              from reclist rl
                             where rl.rlmonth >=
                                   to_char(add_months(sysdate, -12), 'yyyy.mm') 
                                   and rl.RLPFID not like 'B04%'--去除消防用水
                             group by rl.rlsmfid,
                                      rl.rlmonth,
                                      rl.RLPFID,
                                      rl.rlreverseflag,
                                      rl.rlmid) rl1) x
             where x.yf <= 2
             group by rlsmfid, rlmid, rlreverseflag, RLPFID
            having count(*) > 1) rl2
     where ci.ciid = mi.micid
      -- and ci.cismfid = '0201'
       and (mi.mipfid like 'B%' or mi.mipfid like 'C%' or
           mi.mipfid like 'D%' or mi.mipfid like 'E%' or
           mi.mipfid like 'F%')
       and mistatus not in ('7', '28', '31', '32') --去除销户基建用户
       and rl2.rlmid = mi.micid
       and rl2.rlreadsl / rl2.yfc > 500
       and rl2.rlreverseflag = 'N';
  commit;
  DELETE VIEW_BRECLIST1;
  COMMIT;
  insert into VIEW_BRECLIST1
    select vb.用户编号,
           vb.用户名称,
           vb.区域代码,
           vb.用水分类,
           ...
           rl.rlecode,
           rl.rlreadsl
      from VIEW_BCUSTINFO1 vb, reclist rl
     where vb.用户编号 = rl.rlmid
       and rl.rlmonth >= to_char(add_months(sysdate, -12), 'yyyy.mm');
  COMMIT;
  DELETE VIEW_BCUSTINFO@gis_link;
  COMMIT;
  insert into VIEW_BCUSTINFO@gis_link 
  select * from VIEW_BCUSTINFO1;
  commit;
  DELETE VIEW_BRECLIST@gis_link;
  COMMIT;
  insert into VIEW_BRECLIST@gis_link 
  select * from VIEW_BRECLIST1;
  commit;
end;

  --2017.02.01 哈尔滨 污水费调价 !!!
  procedure 水价调整0201 is
    cursor cur_price is 
    select * from pricedetail where pdpfid not in ('A0101','A0102','A0103','A0104','A06','A0107','A0108') and pdpiid = '02' 
      order by pdpfid for update;
  begin
