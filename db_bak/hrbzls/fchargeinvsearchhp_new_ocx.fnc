...
(
           ...
           '    '||tools.fuppernumber(sum(case when pdpiid <>'08' then  pdje else 0 end )  )              C10             ,--合计金额大写            C10
            '￥'||/*tools.fformatnum(sum(pdje)+sum(pdznj)+max(PLSAVINGBQ ) ,2)*/ tools.fformatnum(sum(case when pdpiid <>'08' then  pdje else 0 end ),2)                  C11             ,--合计金额                C11
            fGetPriceText_gtmx(plid)                C12             ,--水费明细1               C12
          case when  pdpiid ='08' and pdje<>0 then '垃圾费:     ￥'||to_char(round(sum(case when pdpiid ='08' then  pdje else 0 end ),2),'0.00')     else '' end            C13             ,--水费明细2               C13
            ...
            fGetUnitPrice_mx(max(rlid),'01,02,03,04,05,06,07|',7)                                                   C28             ,--应收单价                C28
            fGetUnitMoney_gtmx(plid)                                      C29             ,--水费金额                C29
            '暂无启用'                                                         C30             ,--余量                    C30
          TO_CHAR( sum(pdznj),'9990.00')                                                     C31             ,--滞纳金                  C31
         '总计:'|| TO_CHAR( SUM(pdje)+sum(pdznj ),'9999999.00')                                                         C32             ,--手续费                  C32
            ''                                                        C33             ,--上期抄表日期    （本次交钱）        C33
      '上次余数:'||   replace( tools.fformatnum(max(PLSAVINGQC), 2)   ,'-.','-0.')  /* '上次余数:'||tools.fformatnum(max(PLSAVINGQC),2) */                                                     C34             ,--抄表日期  (上次结余)             C34
        '本次余数:'||   replace( tools.fformatnum(max(PLSAVINGQM), 2)   ,'-.','-0.')   /*'本次余数:'||tools.fformatnum(max(PLSAVINGQM),2) */                                                      C35             ,--账务日期 (本次结余)               C35
            '暂无启用'                                                         C36             ,--抄表月份                C36
