CREATE OR REPLACE PROCEDURE HRBZLS.SP_CBJ_IMP is
  v_rowno   number :=0;
  v_rowstr  varchar2(10);
  v_type    varchar2(10);
begin
INSERT INTO tmp_text (OUT_STR) 
WITH  rl_dis AS 
( 
...
SELECT MIID || CHR(9) || --  水表编号C1,
        ...
        (CASE
          WHEN MRIFSUBMIT = 'N' OR MIIFCHARGE = 'N' THEN
           'N'
          ELSE
           'Y'
        END) || CHR(9) || --  计费标志C42,
...
/*       FGET_CBJ_REC(MIID, 'SCJFRQ') || CHR(9) || --  上次缴费日期C60,
        ...
        0 || CHR(9) || --  报警限额上限C66,
        MRID || CHR(9) || --抄表流水号C67
        DECODE(MD.IFDZSB, 'Y', '1', '0') --倒表标志C68
...
/*delete  tmp_text_t;
insert into tmp_text_t select * from tmp_text;
commit;*/
