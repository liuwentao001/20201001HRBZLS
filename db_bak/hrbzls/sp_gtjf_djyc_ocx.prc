CREATE OR REPLACE PROCEDURE HRBZLS."SP_GTJF_DJYC_OCX" (p_pbatch    in varchar2, --实收批次
  ...
  from (
    select max(pmcode) C1, --资料号                  C1
           ...
           '￥:' || replace(tools.fformatnum(sum(decode(pcd, 'DE', 1, -1) *
                                                   (ppayment - pchange)),
                                               2),
                              '-.',
                              '-0.') C11, --合计金额                C11
           ...
           '抄表员：' || max(FGETBFRPER_smfid(mibfid, mismfid)) ||
           max(case
                 when mistatus in ('13', '21') then
                  '  电话：' || FGETBFRPERtel_smfid(mibfid, mismfid)
                 else
                  ''
               end) C26, --抄表员              C26
           '' C27, --抄表水量                C27
           ...
           '上次结余:' ||
           replace(tools.fformatnum(sum(PSAVINGQC), 2), '-.', '-0.') C34, --抄表日期  (上次结余)  '本次结余 '||tools.fformatnum(max(PSAVINGQM),2)            C34
           '本次结余:' ||
           replace(tools.fformatnum(sum(PSAVINGQM), 2), '-.', '-0.') C35, --账务日期 (本次结余)               C35
           '' C36, --抄表月份                C36
           tools.fformatnum(0, 2) C37, --合计应收1                            C37
           '现金' C38, --收费方式                C38
           '' C39, --水费明细3               C39
           '' C40, --预存发生明细            C40
           ...
           '' C49, --用户预留字段9           C49
           '' C50, --用户预留字段10          C50
           MAX(PBATCH) C51, --应收账流水号            C51
           '' C52, --费用项目                C52
           '' C53, --打印员编号              C53
           to_char(PID) C54, --收费员编号              C54
           '' C55, --序号                    C55
           '' C56, --系统预留字段1           C56
           '' C57, --系统预留字段2           C57
           '' C58, --系统预留字段3           C58
     ...
     where pmid = miid
       and pcid = ciid
       and pbatch = p_pbatch
          -- and (p_pid is null or p_pid = pid)
       and PTRANS = 'S'
          -- AND p_printtype = 'F'
     group by PID
    ...
    select null,null,null,null,null,null,null,null,null,null,
           ...
           null,null,null,null,null,null,null,null,null,null,
           null,null,null,null,null,null,null,null,null,null,
           '9999999999',null,null,null,null,null,null,null,null,null
    from dual
    ) order by c51
    ;
begin
