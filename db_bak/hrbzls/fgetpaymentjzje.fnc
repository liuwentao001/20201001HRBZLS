CREATE OR REPLACE FUNCTION HRBZLS."FGETPAYMENTJZJE"
...
begin
   BEGIN
   select COUNT(*),MAX(T.prcreceived  ) into V_COUNT,LCODE from payment t where t.pbatch=p_rlpbatch
   and t.ptrans='S' ;
     IF V_COUNT=1 THEN
       RETURN LCODE;
     ELSE
  ...
  RL1.RLPID,RL2.RLPID from reclist  t where
     rlpbatch=p_rlpbatch  and (rlgroup =1 OR rlgroup =3)
      ...
      if rl1.rlid=rl2.rlid OR  P_TYPE='Z' then
        select  sum( t.prcreceived   )
        into LCODE  from payment t where
        pbatch=p_rlpbatch;

        select  sum( rlje )
        into  v_psavingqc  from reclist t where
        rlpbatch =p_rlpbatch and t.rlgroup=2 ;
