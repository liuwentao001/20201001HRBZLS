CREATE OR REPLACE TRIGGER HRBZLS."TUB_SYSFACEJOIN1" BEFORE UPDATE
OF AFJPID,
   AFJCID
...
BEGIN
  if nvl(fsyspara('data'),'N')='Y' then
    ...
    --  PARENT "SYSFACELIST2" MUST EXIST WHEN UPDATING A CHILD IN "SYSFACEJOIN1"
    IF (:NEW.AFJPID IS NOT NULL) AND (SEQ = 0) THEN
       ...
       if not found then
          errno  := -20003;
          errmsg := 'PARENT DOES NOT EXIST IN "SYSFACELIST2". CANNOT UPDATE CHILD IN "SYSFACEJOIN1".';
          raise integrity_error;
       end if;
    end if;

    --  Parent "METERFACE" must exist when updating a child in "SYSFACEJOIN1"
    if (:new.AFJCID is not null) and (seq = 0) then
       ...
       IF NOT FOUND THEN
          ERRNO  := -20003;
          ERRMSG := 'PARENT DOES NOT EXIST IN "METERFACE". CANNOT UPDATE CHILD IN "SYSFACEJOIN1".';
          RAISE INTEGRITY_ERROR;
       END IF;
    END IF;
