CREATE OR REPLACE PROCEDURE HRBZLS."SP_FCHARGEINVSEARCHHP_NEW_OCX" (
  ...
  select
        ...
        tools.fmid( max(cminfo),3,'N','/')                                 C3              ,--用户地址                C3
          ...
          '污水处理费'             C12             ,--水费明细1               C12
           ...
           tools.fformatnum( max(pddj),2)                C28             ,--应收单价                C28
            ''                                      C29             ,--水费金额                C29
            '                                '                                                         C30             ,--余量                    C30
          '违约金:'||replace( tools.fformatnum( sum(pdznj)  ,2) ,'-.','-0.')            C31             ,--滞纳金                  C31
           ...
           '本次结余:'||   tools.fformatnum( max(qcsaving) + SUM(PLSAVINGBQ ) + max(YC) ,2)                                                     C35             ,--账务日期 (本次结余)               C35
            '用水起止日期:'||MIN(to_char(RLPRDATE,'yyyy-mm-dd'))||' ---- ' ||MAX(to_char(RLRDATE,'yyyy-mm-dd'))                                                         C36             ,--抄表月份                C36
          replace(  tools.fformatnum(  sum(pdje)  ,2)  ,'-.','-0.')                   C37  ,--合计应收1                            C37
            '                                '                                                         C38             ,--收费方式                C38
            '                                '                                                         C39             ,--水费明细3               C39
           '                                '                                                     C40             ,--预存发生明细            C40
           '大写金额: '|| tools.fuppernumber(  sum(pdje)  )                                C41             ,--应收金额大写           C41
...
min(rlscode) rlscode,max(rlecode) rlecode,max(rlsl) rlsl,max(RLADDSL) RLADDSL,MIN(RLDATE) RLDATE, MAX(RLRDATE) RLRDATE,MIN(RLPRDATE) RLPRDATE ,
max(ppayee) ppayee,DECODE(PLCD,'DE',1,-1)*sum(pdje) pdje, DECODE(PLCD,'DE',1,-1)*sum(pdznj) pdznj , max(PLSAVINGBQ ) PLSAVINGBQ,
max(PLSAVINGQC) PLSAVINGQC,max(PLSAVINGQm) PLSAVINGQm, max(pdatetime) pdatetime ,
MAX(PID) PID,MAX(ptrans) ptrans,max(pddj) pddj,
...
select
        ...
        tools.fmid( max(cminfo),3,'N','/')                                 C3              ,--用户地址                C3
          ...
          /* fgetopername( */  max(ppayee) /* )*/                                        C9              ,--收费员                  C9
           ...
           F_chargeinvsearch_recmx_ocx(PBATCH,null,'5')               C12             ,--水费明细1               C12
           '                                '                           C13             ,--水费明细2               C13
            to_char(sysdate ,'yyyy')              C14             ,--系统时间年              C14
            to_char(sysdate ,'mm')                                             C15             ,--系统时间月              C15
            to_char(sysdate ,'dd')                                             C16             ,--系统时间日              C16
          ...
          '违约金:'||replace( tools.fformatnum( sum(pdznj)  ,2) ,'-.','-0.')            C31             ,--滞纳金                  C31
           ...
           '本次结余:'||   tools.fformatnum( max(qcsaving) + SUM(PLSAVINGBQ ) + max(YC) ,2)                                                     C35             ,--账务日期 (本次结余)               C35
            '用水起止日期:'||MIN(to_char(RLPRDATE,'yyyy-mm-dd'))||' ---- ' ||MAX(to_char(RLRDATE,'yyyy-mm-dd'))                                                         C36             ,--抄表月份                C36
        '小写合计: '||replace(  tools.fformatnum(  sum(pdje) - SUM(PD01JE ) ,2)  ,'-.','-0.')                   C37  ,--合计应收1                            C37
            '                                '                                                         C38             ,--收费方式                C38
            '                                '                                                         C39             ,--水费明细3               C39
           '                                '                                                     C40             ,--预存发生明细            C40
           '大写金额: '|| tools.fuppernumber(  sum(pdje) - SUM(PD01JE ) )                                C41             ,--应收金额大写           C41
            '加减水量    :' || tools.fformatnum(SUM(RLADDSL), 0)  C42             ,--备注           C42
            '                                '                                                      C43             ,--应缴滞纳金3           C43
            '                                '                                                        C44             ,--实缴滞纳金4           C44
            '                                '           C45             ,--应缴水费5           C45
...
group by pbatch
  ...
  '实收金额:'||F_PRINTINV_OCX('C10' ,pbatch,max(pmid) )     \*'实收金额:'|| tools.fformatnum(sum(pdje)+sum(pdznj)+sum(PLSAVINGBQ ) + max(YC) - nvl(SUM(PD01JE),0) ,2) *\                   C11             ,--合计金额                C11
           F_chargeinvsearch_recmx_ocx(PBATCH,max(pmid),'3')               C12             ,--水费明细1               C12
           '                                '                           C13             ,--水费明细2               C13
            to_char(sysdate ,'yyyy')              C14             ,--系统时间年              C14
            to_char(sysdate ,'mm')                                             C15             ,--系统时间月              C15
            to_char(sysdate ,'dd')                                             C16             ,--系统时间日              C16
    ...
    select
        ...
        tools.fmid( max(cminfo),3,'N','/')                                 C3              ,--用户地址                C3
          ...
          /* fgetopername( */  max(ppayee) /* )*/                                        C9              ,--收费员                  C9
            ''              C10             ,--合计金额大写            C10
            '实收金额:'|| tools.fformatnum(sum(pdje)+sum(pdznj)+sum(PLSAVINGBQ ) + max(YC) - nvl(SUM(PD01JE),0) ,2)                    C11             ,--合计金额                C11
    --       F_chargeinvsearch_recmx_ocx(PBATCH,null,'2')               C12             ,--水费明细1               C12
             F_chargeinvsearch_recmx_ocx(PBATCH,null,'2')               C12             ,--水费明细1               C12
           '                                '                           C13             ,--水费明细2               C13
            to_char(sysdate ,'yyyy')              C14             ,--系统时间年              C14
            to_char(sysdate ,'mm')                                             C15             ,--系统时间月              C15
...
begin
/*sp_fchargeinvsearch_fp_new_ocx(
p_pbatch  , --实收批次
