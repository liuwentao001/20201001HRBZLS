create or replace package body pg_pj is
  ...
  */
  procedure pg_smsf(p_rlids varchar2, p_fkfs varchar2, o_log out varchar2 )is
  begin    
    --将应收账流水号按用户分组
  ...
  --票据 收费 单用户应收账
  procedure pg_sf(p_rlids varchar2, p_fkfs varchar2)is
    v_pj_id varchar2(10);
  begin
    v_pj_id := seq_paidbatch.nextval;

    insert into pj_inv_info(id, dyfs, status, cplx, fkfs, month, mcode, rlcname, rlcadr,  scode, ecode, sl, kpje, rlid ,cpje01, cpje02, cpje03)
    with rd as (
         select rdid,
    ...
    select v_pj_id, '0', '0','L', p_fkfs, max(rlmonth), rlcid , rlcname, rlcadr, min(rlscode) , max(rlecode), sum(rlsl) ,sum(rlje), p_rlids, sum(je01), sum(je02), sum(je03)
