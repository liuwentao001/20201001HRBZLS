prompt PL/SQL Developer Export User Objects for user SF_USER@10.10.10.72/YSDB
prompt Created by xss on 2021年3月4日
set define off
spool ruoyi.log

prompt
prompt Creating table BANK_CHARGE_TRAN_LOG
prompt ===================================
prompt
@@bank_charge_tran_log.tab
prompt
prompt Creating table BS_ANNEX
prompt =======================
prompt
@@bs_annex.tab
prompt
prompt Creating table BS_BOOKFRAME
prompt ===========================
prompt
@@bs_bookframe.tab
prompt
prompt Creating table BS_BOOKFRAME_HIS
prompt ===============================
prompt
@@bs_bookframe_his.tab
prompt
prompt Creating table BS_CBJH_TEMP
prompt ===========================
prompt
@@bs_cbjh_temp.tab
prompt
prompt Creating table BS_COST
prompt ======================
prompt
@@bs_cost.tab
prompt
prompt Creating table BS_CUSTINFO
prompt ==========================
prompt
@@bs_custinfo.tab
prompt
prompt Creating table BS_CUSTINFO_HIS
prompt ==============================
prompt
@@bs_custinfo_his.tab
prompt
prompt Creating table BS_CUSTINFO_copy1
prompt ================================
prompt
@@bs_custinfo_copy1.tab
prompt
prompt Creating table BS_IMPORTUSERS
prompt =============================
prompt
@@bs_importusers.tab
prompt
prompt Creating table BS_METERDOC
prompt ==========================
prompt
@@bs_meterdoc.tab
prompt
prompt Creating table BS_METERDOC_BAK
prompt ==============================
prompt
@@bs_meterdoc_bak.tab
prompt
prompt Creating table BS_METERDOC_HIS
prompt ==============================
prompt
@@bs_meterdoc_his.tab
prompt
prompt Creating table BS_METERFH_STORE
prompt ===============================
prompt
@@bs_meterfh_store.tab
prompt
prompt Creating table BS_METERINFO
prompt ===========================
prompt
@@bs_meterinfo.tab
prompt
prompt Creating table BS_METERINFO_HIS
prompt ===============================
prompt
@@bs_meterinfo_his.tab
prompt
prompt Creating table BS_METERPICTURE
prompt ==============================
prompt
@@bs_meterpicture.tab
prompt
prompt Creating table BS_METERREAD
prompt ===========================
prompt
@@bs_meterread.tab
prompt
prompt Creating table BS_METERREAD_HIS
prompt ===============================
prompt
@@bs_meterread_his.tab
prompt
prompt Creating table BS_PAYMENT
prompt =========================
prompt
@@bs_payment.tab
prompt
prompt Creating table BS_PAYMENT_TEMP
prompt ==============================
prompt
@@bs_payment_temp.tab
prompt
prompt Creating table BS_PMDETAIL
prompt ==========================
prompt
@@bs_pmdetail.tab
prompt
prompt Creating table BS_PRICEDETAIL
prompt =============================
prompt
@@bs_pricedetail.tab
prompt
prompt Creating table BS_PRICEDETAIL_HIS
prompt =================================
prompt
@@bs_pricedetail_his.tab
prompt
prompt Creating table BS_PRICEFRAME
prompt ============================
prompt
@@bs_priceframe.tab
prompt
prompt Creating table BS_PRICEFRAME_HIS
prompt ================================
prompt
@@bs_priceframe_his.tab
prompt
prompt Creating table BS_PRICESTEP
prompt ===========================
prompt
@@bs_pricestep.tab
prompt
prompt Creating table BS_PRINT_NOTICE
prompt ==============================
prompt
@@bs_print_notice.tab
prompt
prompt Creating table BS_RECDETAIL
prompt ===========================
prompt
@@bs_recdetail.tab
prompt
prompt Creating table BS_RECDETAIL_SSCZ_TEMP
prompt =====================================
prompt
@@bs_recdetail_sscz_temp.tab
prompt
prompt Creating table BS_RECDETAIL_TEMP
prompt ================================
prompt
@@bs_recdetail_temp.tab
prompt
prompt Creating table BS_RECLIST
prompt =========================
prompt
@@bs_reclist.tab
prompt
prompt Creating table BS_RECLIST_SSCZ_TEMP
prompt ===================================
prompt
@@bs_reclist_sscz_temp.tab
prompt
prompt Creating table BS_RECLIST_TEMP
prompt ==============================
prompt
@@bs_reclist_temp.tab
prompt
prompt Creating table BS_STOREINFO
prompt ===========================
prompt
@@bs_storeinfo.tab
prompt
prompt Creating table BS_STOREROOM
prompt ===========================
prompt
@@bs_storeroom.tab
prompt
prompt Creating table BS_ZWBFRPER
prompt ==========================
prompt
@@bs_zwbfrper.tab
prompt
prompt Creating table CHK_METERREAD
prompt ============================
prompt
@@chk_meterread.tab
prompt
prompt Creating table DP_INVOICE_DETAIL_LOG
prompt ====================================
prompt
@@dp_invoice_detail_log.tab
prompt
prompt Creating table DP_INVOICE_LOG
prompt =============================
prompt
@@dp_invoice_log.tab
prompt
prompt Creating table DP_PARAM
prompt =======================
prompt
@@dp_param.tab
prompt
prompt Creating table DP_RED_INVOICE_DETAIL_LOG
prompt ========================================
prompt
@@dp_red_invoice_detail_log.tab
prompt
prompt Creating table DP_RED_INVOICE_LOG
prompt =================================
prompt
@@dp_red_invoice_log.tab
prompt
prompt Creating table DR$TEMP_CONTENT$I
prompt ================================
prompt
@@dr$temp_content$i.tab
prompt
prompt Creating table DR$TEMP_CONTENT$K
prompt ================================
prompt
@@dr$temp_content$k.tab
prompt
prompt Creating table DR$TEMP_CONTENT$N
prompt ================================
prompt
@@dr$temp_content$n.tab
prompt
prompt Creating table DR$TEMP_CONTENT$R
prompt ================================
prompt
@@dr$temp_content$r.tab
prompt
prompt Creating table GD_CUSTCHANGEDT
prompt ==============================
prompt
@@gd_custchangedt.tab
prompt
prompt Creating table GD_CUSTCHANGEHD
prompt ==============================
prompt
@@gd_custchangehd.tab
prompt
prompt Creating table GD_CUSTMETERREGDT
prompt ================================
prompt
@@gd_custmeterregdt.tab
prompt
prompt Creating table GD_CUSTREGHD
prompt ===========================
prompt
@@gd_custreghd.tab
prompt
prompt Creating table GD_METERTGLDT
prompt ============================
prompt
@@gd_metertgldt.tab
prompt
prompt Creating table GD_METERTGLHD
prompt ============================
prompt
@@gd_metertglhd.tab
prompt
prompt Creating table GD_PAIDADJUSTDT
prompt ==============================
prompt
@@gd_paidadjustdt.tab
prompt
prompt Creating table GD_PAIDADJUSTHD
prompt ==============================
prompt
@@gd_paidadjusthd.tab
prompt
prompt Creating table GD_PAY_DAILY_PID
prompt ===============================
prompt
@@gd_pay_daily_pid.tab
prompt
prompt Creating table GD_PAY_DAILY_YXHD
prompt ================================
prompt
@@gd_pay_daily_yxhd.tab
prompt
prompt Creating table GD_RECCZDT
prompt =========================
prompt
@@gd_recczdt.tab
prompt
prompt Creating table GD_RECCZHD
prompt =========================
prompt
@@gd_recczhd.tab
prompt
prompt Creating table GD_RECTRANSDT
prompt ============================
prompt
@@gd_rectransdt.tab
prompt
prompt Creating table GD_RECTRANSHD
prompt ============================
prompt
@@gd_rectranshd.tab
prompt
prompt Creating table GEN_TABLE
prompt ========================
prompt
@@gen_table.tab
prompt
prompt Creating table GEN_TABLE_COLUMN
prompt ===============================
prompt
@@gen_table_column.tab
prompt
prompt Creating table LINB
prompt ===================
prompt
@@linb.tab
prompt
prompt Creating table OPERACCNT_LEVEL
prompt ==============================
prompt
@@operaccnt_level.tab
prompt
prompt Creating table OPERACCNT_TEAM
prompt =============================
prompt
@@operaccnt_team.tab
prompt
prompt Creating table PAY_PARA_TMP
prompt ===========================
prompt
@@pay_para_tmp.tab
prompt
prompt Creating table PJ_INVSTOCK
prompt ==========================
prompt
@@pj_invstock.tab
prompt
prompt Creating table PJ_INVSTOCK_SP
prompt =============================
prompt
@@pj_invstock_sp.tab
prompt
prompt Creating table PJ_INV_DETAIL
prompt ============================
prompt
@@pj_inv_detail.tab
prompt
prompt Creating table PJ_INV_DETAIL_SP
prompt ===============================
prompt
@@pj_inv_detail_sp.tab
prompt
prompt Creating table PJ_INV_INFO
prompt ==========================
prompt
@@pj_inv_info.tab
prompt
prompt Creating table PJ_INV_INFO_SP
prompt =============================
prompt
@@pj_inv_info_sp.tab
prompt
prompt Creating table QRTZ_JOB_DETAILS
prompt ===============================
prompt
@@qrtz_job_details.tab
prompt
prompt Creating table QRTZ_TRIGGERS
prompt ============================
prompt
@@qrtz_triggers.tab
prompt
prompt Creating table QRTZ_BLOB_TRIGGERS
prompt =================================
prompt
@@qrtz_blob_triggers.tab
prompt
prompt Creating table QRTZ_CALENDARS
prompt =============================
prompt
@@qrtz_calendars.tab
prompt
prompt Creating table QRTZ_CRON_TRIGGERS
prompt =================================
prompt
@@qrtz_cron_triggers.tab
prompt
prompt Creating table QRTZ_FIRED_TRIGGERS
prompt ==================================
prompt
@@qrtz_fired_triggers.tab
prompt
prompt Creating table QRTZ_LOCKS
prompt =========================
prompt
@@qrtz_locks.tab
prompt
prompt Creating table QRTZ_PAUSED_TRIGGER_GRPS
prompt =======================================
prompt
@@qrtz_paused_trigger_grps.tab
prompt
prompt Creating table QRTZ_SCHEDULER_STATE
prompt ===================================
prompt
@@qrtz_scheduler_state.tab
prompt
prompt Creating table QRTZ_SIMPLE_TRIGGERS
prompt ===================================
prompt
@@qrtz_simple_triggers.tab
prompt
prompt Creating table QRTZ_SIMPROP_TRIGGERS
prompt ====================================
prompt
@@qrtz_simprop_triggers.tab
prompt
prompt Creating table RELATION_JZGD_DZGD
prompt =================================
prompt
@@relation_jzgd_dzgd.tab
prompt
prompt Creating table RELATION_PAYMENT_JZGD
prompt ====================================
prompt
@@relation_payment_jzgd.tab
prompt
prompt Creating table REQUEST_BJSF
prompt ===========================
prompt
@@request_bjsf.tab
prompt
prompt Creating table REQUEST_CB
prompt =========================
prompt
@@request_cb.tab
prompt
prompt Creating table REQUEST_CJDY
prompt ===========================
prompt
@@request_cjdy.tab
prompt
prompt Creating table REQUEST_DHGL
prompt ===========================
prompt
@@request_dhgl.tab
prompt
prompt Creating table REQUEST_DZ
prompt =========================
prompt
@@request_dz.tab
prompt
prompt Creating table REQUEST_DZGD
prompt ===========================
prompt
@@request_dzgd.tab
prompt
prompt Creating table REQUEST_GDL
prompt ==========================
prompt
@@request_gdl.tab
prompt
prompt Creating table REQUEST_GH
prompt =========================
prompt
@@request_gh.tab
prompt
prompt Creating table REQUEST_GZHB
prompt ===========================
prompt
@@request_gzhb.tab
prompt
prompt Creating table REQUEST_HTGL
prompt ===========================
prompt
@@request_htgl.tab
prompt
prompt Creating table REQUEST_JZGD
prompt ===========================
prompt
@@request_jzgd.tab
prompt
prompt Creating table REQUEST_JZGL
prompt ===========================
prompt
@@request_jzgl.tab
prompt
prompt Creating table REQUEST_MMCZ
prompt ===========================
prompt
@@request_mmcz.tab
prompt
prompt Creating table REQUEST_PID
prompt ==========================
prompt
@@request_pid.tab
prompt
prompt Creating table REQUEST_PJXX
prompt ===========================
prompt
@@request_pjxx.tab
prompt
prompt Creating table REQUEST_QYDZ
prompt ===========================
prompt
@@request_qydz.tab
prompt
prompt Creating table REQUEST_SBDA
prompt ===========================
prompt
@@request_sbda.tab
prompt
prompt Creating table REQUEST_SBDZ
prompt ===========================
prompt
@@request_sbdz.tab
prompt
prompt Creating table REQUEST_SBLC
prompt ===========================
prompt
@@request_sblc.tab
prompt
prompt Creating table REQUEST_SFFS
prompt ===========================
prompt
@@request_sffs.tab
prompt
prompt Creating table REQUEST_SGKG
prompt ===========================
prompt
@@request_sgkg.tab
prompt
prompt Creating table REQUEST_SJBG
prompt ===========================
prompt
@@request_sjbg.tab
prompt
prompt Creating table REQUEST_SSCZ
prompt ===========================
prompt
@@request_sscz.tab
prompt
prompt Creating table REQUEST_XH
prompt =========================
prompt
@@request_xh.tab
prompt
prompt Creating table REQUEST_YCTF
prompt ===========================
prompt
@@request_yctf.tab
prompt
prompt Creating table REQUEST_YHDBSB
prompt =============================
prompt
@@request_yhdbsb.tab
prompt
prompt Creating table REQUEST_YHDBYH
prompt =============================
prompt
@@request_yhdbyh.tab
prompt
prompt Creating table REQUEST_YHXX
prompt ===========================
prompt
@@request_yhxx.tab
prompt
prompt Creating table REQUEST_YHZT
prompt ===========================
prompt
@@request_yhzt.tab
prompt
prompt Creating table REQUEST_YSCZ
prompt ===========================
prompt
@@request_yscz.tab
prompt
prompt Creating table REQUEST_YXHD
prompt ===========================
prompt
@@request_yxhd.tab
prompt
prompt Creating table REQUEST_ZBSM
prompt ===========================
prompt
@@request_zbsm.tab
prompt
prompt Creating table REQUEST_ZLSF
prompt ===========================
prompt
@@request_zlsf.tab
prompt
prompt Creating table REQUEST_ZLSFHIS
prompt ==============================
prompt
@@request_zlsfhis.tab
prompt
prompt Creating table REQUEST_ZQGZ
prompt ===========================
prompt
@@request_zqgz.tab
prompt
prompt Creating table SMS_EXCLUDE
prompt ==========================
prompt
@@sms_exclude.tab
prompt
prompt Creating table SMS_LOGLIST
prompt ==========================
prompt
@@sms_loglist.tab
prompt
prompt Creating table SMS_MONTH_LOG
prompt ============================
prompt
@@sms_month_log.tab
prompt
prompt Creating table SMS_PARAM
prompt ========================
prompt
@@sms_param.tab
prompt
prompt Creating table SMS_TEMPLATE
prompt ===========================
prompt
@@sms_template.tab
prompt
prompt Creating table SYSMANAPARA
prompt ==========================
prompt
@@sysmanapara.tab
prompt
prompt Creating table SYSMETERTYPE
prompt ===========================
prompt
@@sysmetertype.tab
prompt
prompt Creating table SYS_CONFIG
prompt =========================
prompt
@@sys_config.tab
prompt
prompt Creating table SYS_DEPT
prompt =======================
prompt
@@sys_dept.tab
prompt
prompt Creating table SYS_DICT_DATA
prompt ============================
prompt
@@sys_dict_data.tab
prompt
prompt Creating table SYS_DICT_DATA_BAK
prompt ================================
prompt
@@sys_dict_data_bak.tab
prompt
prompt Creating table SYS_DICT_TYPE
prompt ============================
prompt
@@sys_dict_type.tab
prompt
prompt Creating table SYS_DISTRICTINFO
prompt ===============================
prompt
@@sys_districtinfo.tab
prompt
prompt Creating table SYS_GUIDE
prompt ========================
prompt
@@sys_guide.tab
prompt
prompt Creating table SYS_JOB
prompt ======================
prompt
@@sys_job.tab
prompt
prompt Creating table SYS_JOB_LOG
prompt ==========================
prompt
@@sys_job_log.tab
prompt
prompt Creating table SYS_LOGININFOR
prompt =============================
prompt
@@sys_logininfor.tab
prompt
prompt Creating table SYS_MENU
prompt =======================
prompt
@@sys_menu.tab
prompt
prompt Creating table SYS_NOTICE
prompt =========================
prompt
@@sys_notice.tab
prompt
prompt Creating table SYS_OPER_LOG
prompt ===========================
prompt
@@sys_oper_log.tab
prompt
prompt Creating table SYS_PARA
prompt =======================
prompt
@@sys_para.tab
prompt
prompt Creating table SYS_POST
prompt =======================
prompt
@@sys_post.tab
prompt
prompt Creating table SYS_ROLE
prompt =======================
prompt
@@sys_role.tab
prompt
prompt Creating table SYS_ROLE_DEPT
prompt ============================
prompt
@@sys_role_dept.tab
prompt
prompt Creating table SYS_ROLE_MENU
prompt ============================
prompt
@@sys_role_menu.tab
prompt
prompt Creating table SYS_SEQLIST
prompt ==========================
prompt
@@sys_seqlist.tab
prompt
prompt Creating table SYS_STREETMANAGE
prompt ===============================
prompt
@@sys_streetmanage.tab
prompt
prompt Creating table SYS_USER
prompt =======================
prompt
@@sys_user.tab
prompt
prompt Creating table SYS_USERNO
prompt =========================
prompt
@@sys_userno.tab
prompt
prompt Creating table SYS_USER_ONLINE
prompt ==============================
prompt
@@sys_user_online.tab
prompt
prompt Creating table SYS_USER_POST
prompt ============================
prompt
@@sys_user_post.tab
prompt
prompt Creating table SYS_USER_ROLE
prompt ============================
prompt
@@sys_user_role.tab
prompt
prompt Creating table WECHAT_ACCESS_TOKEN
prompt ==================================
prompt
@@wechat_access_token.tab
prompt
prompt Creating table WECHAT_ADMIN
prompt ===========================
prompt
@@wechat_admin.tab
prompt
prompt Creating table WECHAT_PUSH_IP
prompt =============================
prompt
@@wechat_push_ip.tab
prompt
prompt Creating table WECHAT_PUSH_TEMP
prompt ===============================
prompt
@@wechat_push_temp.tab
prompt
prompt Creating table WECHAT_USER
prompt ==========================
prompt
@@wechat_user.tab
prompt
prompt Creating table WF_FLOWMAIN
prompt ==========================
prompt
@@wf_flowmain.tab
prompt
prompt Creating table WF_FLOWCCROLE
prompt ============================
prompt
@@wf_flowccrole.tab
prompt
prompt Creating table WF_FLOWLINE
prompt ==========================
prompt
@@wf_flowline.tab
prompt
prompt Creating table WF_FLOWNODE
prompt ==========================
prompt
@@wf_flownode.tab
prompt
prompt Creating table WF_FLOWNODEROLE
prompt ==============================
prompt
@@wf_flownoderole.tab
prompt
prompt Creating table WF_FRMATTACH
prompt ===========================
prompt
@@wf_frmattach.tab
prompt
prompt Creating table WF_FRMATTR
prompt =========================
prompt
@@wf_frmattr.tab
prompt
prompt Creating table WF_FRMDETAIL
prompt ===========================
prompt
@@wf_frmdetail.tab
prompt
prompt Creating table WF_FRMMAIN
prompt =========================
prompt
@@wf_frmmain.tab
prompt
prompt Creating table WF_FRMNODERELATION
prompt =================================
prompt
@@wf_frmnoderelation.tab
prompt
prompt Creating table WF_INSTANCE
prompt ==========================
prompt
@@wf_instance.tab
prompt
prompt Creating table WF_NODEBUTTONRELATION
prompt ====================================
prompt
@@wf_nodebuttonrelation.tab
prompt
prompt Creating table WF_TASK
prompt ======================
prompt
@@wf_task.tab
prompt
prompt Creating table YYT_BANNER
prompt =========================
prompt
@@yyt_banner.tab
prompt
prompt Creating table YYT_BUSINESSMAP
prompt ==============================
prompt
@@yyt_businessmap.tab
prompt
prompt Creating table YYT_MENU
prompt =======================
prompt
@@yyt_menu.tab
prompt
prompt Creating table YYT_OPINION
prompt ==========================
prompt
@@yyt_opinion.tab
prompt
prompt Creating table YYT_USER
prompt =======================
prompt
@@yyt_user.tab
prompt
prompt Creating table YYT_WATERCUTOFF
prompt ==============================
prompt
@@yyt_watercutoff.tab
prompt
prompt Creating table ZD_CHEQUE
prompt ========================
prompt
@@zd_cheque.tab
prompt
prompt Creating table ZD_INVOICE
prompt =========================
prompt
@@zd_invoice.tab
prompt
prompt Creating table ZD_PAY_DAILY_PID
prompt ===============================
prompt
@@zd_pay_daily_pid.tab
prompt
prompt Creating table ZD_PAY_DAILY_YXHD
prompt ================================
prompt
@@zd_pay_daily_yxhd.tab
prompt
prompt Creating table ZD_STPAYMENTCWDZREGHD
prompt ====================================
prompt
@@zd_stpaymentcwdzreghd.tab
prompt
prompt Creating table ZD_STPAYMENTYXDZREGHD
prompt ====================================
prompt
@@zd_stpaymentyxdzreghd.tab
prompt
prompt Creating sequence ACT_EVT_LOG_SEQ
prompt =================================
prompt
@@act_evt_log_seq.seq
prompt
prompt Creating sequence SEQIMUSERID
prompt =============================
prompt
@@seqimuserid.seq
prompt
prompt Creating sequence SEQMESTERDOCID
prompt ================================
prompt
@@seqmesterdocid.seq
prompt
prompt Creating sequence SEQPRICEID
prompt ============================
prompt
@@seqpriceid.seq
prompt
prompt Creating sequence SEQUSERNO
prompt ===========================
prompt
@@sequserno.seq
prompt
prompt Creating sequence SEQ_BS_CUSTINFO
prompt =================================
prompt
@@seq_bs_custinfo.seq
prompt
prompt Creating sequence SEQ_BS_IMPORTUSERS
prompt ====================================
prompt
@@seq_bs_importusers.seq
prompt
prompt Creating sequence SEQ_BS_METERFH_STORE
prompt ======================================
prompt
@@seq_bs_meterfh_store.seq
prompt
prompt Creating sequence SEQ_CUSTINFO
prompt ==============================
prompt
@@seq_custinfo.seq
prompt
prompt Creating sequence SEQ_DZGD
prompt ==========================
prompt
@@seq_dzgd.seq
prompt
prompt Creating sequence SEQ_EVENT_BK
prompt ==============================
prompt
@@seq_event_bk.seq
prompt
prompt Creating sequence SEQ_GEN_TABLE
prompt ===============================
prompt
@@seq_gen_table.seq
prompt
prompt Creating sequence SEQ_GEN_TABLE_COLUMN
prompt ======================================
prompt
@@seq_gen_table_column.seq
prompt
prompt Creating sequence SEQ_JZGD
prompt ==========================
prompt
@@seq_jzgd.seq
prompt
prompt Creating sequence SEQ_LOGID
prompt ===========================
prompt
@@seq_logid.seq
prompt
prompt Creating sequence SEQ_METERREAD
prompt ===============================
prompt
@@seq_meterread.seq
prompt
prompt Creating sequence SEQ_METERREADM
prompt ================================
prompt
@@seq_meterreadm.seq
prompt
prompt Creating sequence SEQ_MRID
prompt ==========================
prompt
@@seq_mrid.seq
prompt
prompt Creating sequence SEQ_PAIDBATCH
prompt ===============================
prompt
@@seq_paidbatch.seq
prompt
prompt Creating sequence SEQ_PAIDMENT
prompt ==============================
prompt
@@seq_paidment.seq
prompt
prompt Creating sequence SEQ_PJ_INV_INFO
prompt =================================
prompt
@@seq_pj_inv_info.seq
prompt
prompt Creating sequence SEQ_RECLIST
prompt =============================
prompt
@@seq_reclist.seq
prompt
prompt Creating sequence SEQ_SMS_LOGLIST
prompt =================================
prompt
@@seq_sms_loglist.seq
prompt
prompt Creating sequence SEQ_SMS_MONTH_LOG
prompt ===================================
prompt
@@seq_sms_month_log.seq
prompt
prompt Creating sequence SEQ_SYS_CONFIG
prompt ================================
prompt
@@seq_sys_config.seq
prompt
prompt Creating sequence SEQ_SYS_DEPT
prompt ==============================
prompt
@@seq_sys_dept.seq
prompt
prompt Creating sequence SEQ_SYS_DICT_DATA
prompt ===================================
prompt
@@seq_sys_dict_data.seq
prompt
prompt Creating sequence SEQ_SYS_DICT_TYPE
prompt ===================================
prompt
@@seq_sys_dict_type.seq
prompt
prompt Creating sequence SEQ_SYS_JOB
prompt =============================
prompt
@@seq_sys_job.seq
prompt
prompt Creating sequence SEQ_SYS_JOB_LOG
prompt =================================
prompt
@@seq_sys_job_log.seq
prompt
prompt Creating sequence SEQ_SYS_LOGININFOR
prompt ====================================
prompt
@@seq_sys_logininfor.seq
prompt
prompt Creating sequence SEQ_SYS_MENU
prompt ==============================
prompt
@@seq_sys_menu.seq
prompt
prompt Creating sequence SEQ_SYS_NOTICE
prompt ================================
prompt
@@seq_sys_notice.seq
prompt
prompt Creating sequence SEQ_SYS_OPER_LOG
prompt ==================================
prompt
@@seq_sys_oper_log.seq
prompt
prompt Creating sequence SEQ_SYS_POST
prompt ==============================
prompt
@@seq_sys_post.seq
prompt
prompt Creating sequence SEQ_SYS_PRICEFRAME
prompt ====================================
prompt
@@seq_sys_priceframe.seq
prompt
prompt Creating sequence SEQ_SYS_ROLE
prompt ==============================
prompt
@@seq_sys_role.seq
prompt
prompt Creating sequence SEQ_SYS_USER
prompt ==============================
prompt
@@seq_sys_user.seq
prompt
prompt Creating sequence SEQ_WF_FLOWMAIN
prompt =================================
prompt
@@seq_wf_flowmain.seq
prompt
prompt Creating sequence SEQ_WF_FRMMAIN
prompt ================================
prompt
@@seq_wf_frmmain.seq
prompt
prompt Creating sequence SEQ_YYT_MENU
prompt ==============================
prompt
@@seq_yyt_menu.seq
prompt
prompt Creating sequence SEQ_YYT_USER
prompt ==============================
prompt
@@seq_yyt_user.seq
prompt
prompt Creating sequence SEQ_YYT_WATERCUTOFF
prompt =====================================
prompt
@@seq_yyt_watercutoff.seq
prompt
prompt Creating sequence SYS_DICT_DATA_DICT_CODE_SEQ
prompt =============================================
prompt
@@sys_dict_data_dict_code_seq.seq
prompt
prompt Creating sequence SYS_DICT_TYPE_DICT_ID_SEQ
prompt ===========================================
prompt
@@sys_dict_type_dict_id_seq.seq
prompt
prompt Creating view TMP_VI_DZGL
prompt =========================
prompt
@@tmp_vi_dzgl.vw
prompt
prompt Creating view VIEW_CHAOBIAO
prompt ===========================
prompt
@@view_chaobiao.vw
prompt
prompt Creating view VIEW_CUSTINFO
prompt ===========================
prompt
@@view_custinfo.vw
prompt
prompt Creating view VIEW_CUSTMETERINFO
prompt ================================
prompt
@@view_custmeterinfo.vw
prompt
prompt Creating view VIEW_PAYMENT
prompt ==========================
prompt
@@view_payment.vw
prompt
prompt Creating view VIEW_ZKH
prompt ======================
prompt
@@view_zkh.vw
prompt
prompt Creating view VI_BOOKFRAME
prompt ==========================
prompt
@@vi_bookframe.vw
prompt
prompt Creating view VI_ZHUILIANG
prompt ==========================
prompt
@@vi_zhuiliang.vw
prompt
prompt Creating package PD_DHZ
prompt =======================
prompt
@@pd_dhz.spc
prompt
prompt Creating package PG_CB_COST
prompt ===========================
prompt
@@pg_cb_cost.spc
prompt
prompt Creating package PG_CHKOUT
prompt ==========================
prompt
@@pg_chkout.spc
prompt
prompt Creating package PG_DHZ
prompt =======================
prompt
@@pg_dhz.spc
prompt
prompt Creating package PG_EWIDE_METERTRANS
prompt ====================================
prompt
@@pg_ewide_metertrans.spc
prompt
prompt Creating package PG_EWIDE_RAEDPLAN_01
prompt =====================================
prompt
@@pg_ewide_raedplan_01.spc
prompt
prompt Creating package PG_INSERT
prompt ==========================
prompt
@@pg_insert.spc
prompt
prompt Creating package PG_METERTRANS
prompt ==============================
prompt
@@pg_metertrans.spc
prompt
prompt Creating package PG_METER_READ
prompt ==============================
prompt
@@pg_meter_read.spc
prompt
prompt Creating package PG_PAID
prompt ========================
prompt
@@pg_paid.spc
prompt
prompt Creating package PG_PJ
prompt ======================
prompt
@@pg_pj.spc
prompt
prompt Creating package PG_RAEDPLAN
prompt ============================
prompt
@@pg_raedplan.spc
prompt
prompt Creating package PG_RECTRANS
prompt ============================
prompt
@@pg_rectrans.spc
prompt
prompt Creating package PG_UPDATE
prompt ==========================
prompt
@@pg_update.spc
prompt
prompt Creating package TOOLS
prompt ======================
prompt
@@tools.spc
prompt
prompt Creating type CONNSTRIMPL
prompt =========================
prompt
@@connstrimpl.tps
prompt
prompt Creating function CONNSTR
prompt =========================
prompt
@@connstr.fnc
prompt
prompt Creating function FCHKMETERNEEDCHARGE
prompt =====================================
prompt
@@fchkmeterneedcharge.fnc
prompt
prompt Creating function FCHKMETERNEEDREAD
prompt ===================================
prompt
@@fchkmeterneedread.fnc
prompt
prompt Creating function FGETCUSTNAME
prompt ==============================
prompt
@@fgetcustname.fnc
prompt
prompt Creating function FGETMETERINFO
prompt ===============================
prompt
@@fgetmeterinfo.fnc
prompt
prompt Creating function FGETSEQUENCE
prompt ==============================
prompt
@@fgetsequence.fnc
prompt
prompt Creating function FGET_TZDJ
prompt ===========================
prompt
@@fget_tzdj.fnc
prompt
prompt Creating function FGET调整单价
prompt ==========================
prompt
@@fget调整单价.fnc
prompt
prompt Creating function FN_GET_CIID
prompt =============================
prompt
@@fn_get_ciid.fnc
prompt
prompt Creating function FPARA
prompt =======================
prompt
@@fpara.fnc
prompt
prompt Creating function MD5
prompt =====================
prompt
@@md5.fnc
prompt
prompt Creating procedure LIN
prompt ======================
prompt
@@lin.prc
prompt
prompt Creating procedure PROC_DELETEZONGBIAO
prompt ======================================
prompt
@@proc_deletezongbiao.prc
prompt
prompt Creating procedure PROC_INSERTMETER
prompt ===================================
prompt
@@proc_insertmeter.prc
prompt
prompt Creating procedure PROC_JZ
prompt ==========================
prompt
@@proc_jz.prc
prompt
prompt Creating procedure PROC_METERCHANAGE
prompt ====================================
prompt
@@proc_meterchanage.prc
prompt
prompt Creating procedure PROC_METERREAD_BATCH_MAKE
prompt ============================================
prompt
@@proc_meterread_batch_make.prc
prompt
prompt Creating procedure PROC_SPLIT_REQUEST_JZGL
prompt ==========================================
prompt
@@proc_split_request_jzgl.prc
prompt
prompt Creating procedure SP_CANCELUSERPROC
prompt ====================================
prompt
@@sp_canceluserproc.prc
prompt
prompt Creating procedure SP_CBPROC
prompt ============================
prompt
@@sp_cbproc.prc
prompt
prompt Creating procedure SP_METERINFO_MIRORDER
prompt ========================================
prompt
@@sp_meterinfo_mirorder.prc
prompt
prompt Creating procedure SP_JZGLAUDIT
prompt ===============================
prompt
@@sp_jzglaudit.prc
prompt
prompt Creating procedure SP_MRSLCHECK_HRB
prompt ===================================
prompt
@@sp_mrslcheck_hrb.prc
prompt
prompt Creating procedure SP_UPDATEYHZT
prompt ================================
prompt
@@sp_updateyhzt.prc
prompt
prompt Creating procedure TEST
prompt =======================
prompt
@@test.prc
prompt
prompt Creating package body PG_CB_COST
prompt ================================
prompt
@@pg_cb_cost.bdy
prompt
prompt Creating package body PG_CHKOUT
prompt ===============================
prompt
@@pg_chkout.bdy
prompt
prompt Creating package body PG_DHZ
prompt ============================
prompt
@@pg_dhz.bdy
prompt
prompt Creating package body PG_EWIDE_METERTRANS
prompt =========================================
prompt
@@pg_ewide_metertrans.bdy
prompt
prompt Creating package body PG_EWIDE_RAEDPLAN_01
prompt ==========================================
prompt
@@pg_ewide_raedplan_01.bdy
prompt
prompt Creating package body PG_INSERT
prompt ===============================
prompt
@@pg_insert.bdy
prompt
prompt Creating package body PG_METERTRANS
prompt ===================================
prompt
@@pg_metertrans.bdy
prompt
prompt Creating package body PG_METER_READ
prompt ===================================
prompt
@@pg_meter_read.bdy
prompt
prompt Creating package body PG_PAID
prompt =============================
prompt
@@pg_paid.bdy
prompt
prompt Creating package body PG_PJ
prompt ===========================
prompt
@@pg_pj.bdy
prompt
prompt Creating package body PG_RAEDPLAN
prompt =================================
prompt
@@pg_raedplan.bdy
prompt
prompt Creating package body PG_RECTRANS
prompt =================================
prompt
@@pg_rectrans.bdy
prompt
prompt Creating package body PG_UPDATE
prompt ===============================
prompt
@@pg_update.bdy
prompt
prompt Creating package body TOOLS
prompt ===========================
prompt
@@tools.bdy
prompt
prompt Creating type body CONNSTRIMPL
prompt ==============================
prompt
@@connstrimpl.tpb
prompt
prompt Creating trigger SMS_SEND_INSERT
prompt ================================
prompt
@@sms_send_insert.trg
prompt
prompt Creating trigger TRI_SYS_DICT_TYPE
prompt ==================================
prompt
@@tri_sys_dict_type.trg

prompt Done
spool off
set define on
