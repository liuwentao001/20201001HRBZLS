...
|v_sb.sbifmp|i.sbifmp|混合用水标志(Y-是,N-否 )|
...
|v_sb.sbrcodechar|i.sbrcodechar|本期读数|
...
ys_zw_arlist.arbadflag	呆帐标志（Y :呆坏帐，O:呆坏帐审批中，N:正常帐）
...
```
for i in (select a.aroutflag	--发出标志(Y-发出 N-未发出)
